"""Train a CUDA CNN using one Distorted CDM sample per OFDM subcarrier."""
...
SUBCARRIERS_PER_FRAME = 123
FILE_PATTERN = re.compile(r"frame_(\d+)_sc(\d+)\.mat$")
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--data-root",
        type=Path,
        default=base / "dataset_cdm_sc1_single" / "dataset",
    )
    parser.add_argument(
        "--output-dir",
        type=Path,
        default=base / "training_cdm_sc1_single_channel_cuda",
    )
...
class IndependentSubcarrierCDMDataset(Dataset):
    def __init__(self, split_dir: Path, augment: bool):
        ...
        for label, modulation in enumerate(CLASS_NAMES):
            ...
            for path in sorted(class_dir.glob("frame_*_sc*.mat")):
        ...
        if not self.records:
            raise RuntimeError(f"No independent-subcarrier CDM files under {split_dir}")
    ...
    def __getitem__(
        ...
        _, label, frame_id, subcarrier_id = self.records[index]
...
def predict(
    ...
    for features, labels, frame_ids, subcarrier_ids in loader:
        ...
        result["subcarrier_ids"].extend(subcarrier_ids.tolist())
...
def fuse_frame_predictions(sample_result: dict[str, list]) -> dict[str, list]:
    ...
    for (label, frame_id), probabilities in sorted(grouped.items()):
        if len(probabilities) != SUBCARRIERS_PER_FRAME:
            raise RuntimeError(
                f"Class {CLASS_NAMES[label]} frame {frame_id} has "
                f"{len(probabilities)} subcarriers instead of "
                f"{SUBCARRIERS_PER_FRAME}."
            )
...
def save_training_curves(output_dir: Path, history: list[dict[str, float]]) -> None:
    ...
    axes[1].set(
        xlabel="Epoch",
        ylabel="Accuracy",
        title="Independent-subcarrier accuracy",
        ylim=(0, 1.02),
    )
    axes[1].legend()
...
def write_predictions(
    path: Path, result: dict[str, list], include_subcarrier: bool
) -> None:
    fields = ["frame_id"]
    if include_subcarrier:
        fields.append("subcarrier_id")
    ...
    with path.open("w", newline="", encoding="utf-8") as file:
        ...
        for index in range(len(result["labels"])):
            ...
            if include_subcarrier:
                row["subcarrier_id"] = result["subcarrier_ids"][index]
            row.update(
                {
...
def main() -> None:
    ...
    train_dataset = IndependentSubcarrierCDMDataset(
        args.data_root / "train", augment=True
    )
    val_dataset = IndependentSubcarrierCDMDataset(
        args.data_root / "val", augment=False
    )
    ...
    print(f"Training independent-subcarrier samples: {len(train_dataset)}")
    print(f"Validation independent-subcarrier samples: {len(val_dataset)}")
            ...
            torch.save(
                {
                    ...
                    "best_independent_subcarrier_val_accuracy": best_accuracy,
    ...
    write_predictions(
        args.output_dir / "subcarrier_predictions.csv",
        sample_result,
        include_subcarrier=True,
    )
    write_predictions(
        args.output_dir / "frame_fused_predictions.csv",
        frame_result,
        include_subcarrier=False,
    )
    save_training_curves(args.output_dir, history)
    save_confusion_matrix(
        args.output_dir / "subcarrier_confusion_matrix.png",
        sample_result["labels"],
        sample_result["predictions"],
        "Validation: independent subcarrier samples",
    )
    save_confusion_matrix(
        args.output_dir / "frame_fused_confusion_matrix.png",
        frame_result["labels"],
        frame_result["predictions"],
        "Validation: 123-subcarrier probability fusion",
    )

    metrics = {
        "device": torch.cuda.get_device_name(0),
        "best_epoch": best_epoch,
        "best_independent_subcarrier_validation_accuracy": best_accuracy,
        "final_best_checkpoint_subcarrier_accuracy": sample_accuracy,
        "frame_fused_validation_accuracy": frame_accuracy,
        "training_subcarrier_samples": len(train_dataset),
        "validation_subcarrier_samples": len(val_dataset),
        "validation_frames": len(frame_result["labels"]),
        "subcarriers_per_frame": SUBCARRIERS_PER_FRAME,
        "subcarrier_classification_report": sample_report,
    ...
    print(f"Best independent-subcarrier accuracy: {sample_accuracy:.4f}")
    print(f"123-subcarrier fused frame accuracy: {frame_accuracy:.4f}")
