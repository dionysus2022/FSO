...
% a=repmat(idx_pilots,1,nAB);
% c=rectpulse(1:nAB,nPilots);
% b= B*(rectpulse(1:nAB,nPilots) - 1)  %rectpulse将1重复nPilots次，2重复nPilots次。。。
...
switch pilotOption
    case 'outerQPSK'
        C_pilot = C(abs(C) == max(abs(C)));%取QAM最外面四个点自然构成了一个QPSK
    case 'innerQPSK'
...
for n = 1:nPol
    Stx_pilots(n,idx_pilots(n,:)) = Stx_pilot(n,:);%按照idx_pilots的索引位置安插QPSK导频进去
