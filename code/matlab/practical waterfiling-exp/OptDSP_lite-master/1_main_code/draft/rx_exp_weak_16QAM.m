% rx_exp_weak_16QAM.m - 单帧解调，支持 16QAM 及更高阶调制
% 基于 rx_exp_weak.m，修复 symDemapper 维度不匹配问题
% 核心修复：SIG.M 和 C 必须与实际数据调制格式一致
...
%% File paths
rx_bin_file = 'D:\Project_code\OptDSP_FSO_2\practical waterfiling-exp\OptDSP_lite-master\2_Data_Results\rx_data\2026.06.27\1.bin';
ref_mat_file = 'D:\Project_code\OptDSP_FSO_2\practical waterfiling-exp\OptDSP_lite-master\2_Data_Results\tx_1frame_6mod0607\256QAM\sub01\sig_0001.mat';
scope_Fs = 80e9;
...
% 关键修复：C 必须与 data_tx 的实际调制格式一致
% 使用 TX.QAM.IQmap（已根据 SIG.M 正确配置）
C = TX.QAM.IQmap;
...
%% 6. Demapper + SNR
fprintf('[4] Demapper + SNR (%dQAM)\n', SIG.M);
DSP.DEMAPPER.normalizeTX = false;  % 高阶QAM时 uniquetol 实虚部唯一值数量不等，跳过
for i=1:123
