% tx_1frame_5mod.m - single frame，200帧信号——500帧
...
SIG.rollOff = 0.25; SIG.nPol = 1; SIG.nSyms = 2^9/co; nSpS = 5; laserLW = 0e6;
...
N_per_mod = 1;   % 快速测试：每种调制生成2个信号
N_per_sub = 1;   % 每个子文件夹放1个信号
N_sub = 1;      % 10个子文件夹

out_root = 'D:\Project_code\OptDSP_FSO_2\practical waterfiling-exp\OptDSP_lite-master\2_Data_Results\tx_1frame512';
...
for m = 1:length(mod_bits)
    ...
    % =============================================================
    if bits <= 2
        nBpS_net = bits;        % 2QAM 和 4QAM 没有幅度变化，直接拉满
    else
        nBpS_net = bits - 0.2;  % 16/64/256QAM 减去 0.2 bit 开销，留出正负号余量
    end
