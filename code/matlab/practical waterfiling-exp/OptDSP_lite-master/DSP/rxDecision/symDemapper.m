...
if any(any(isnan(Srx),2))% || ~all(sum(Srx,2))%后半个条件不知道什么意思，因为阻碍了程序运行，故取消
...
if normalizeTX
    for n = 1:nPol
        C_u = (uniquetol(real(C(:,n)),1e-6) + ...
            1j*uniquetol(imag(C(:,n)),1e-6)).';     %uniquetol是在容差1e-6内找到唯一的，也就是找到实部和虚部中唯一的那个幅值，即看看有几种实虚电平和组合,四个象限情况都一样，只考虑一个象限就可以
        Stx_u = uniquetol(real(Stx(n,:)),1e-6) + ...
            1j*uniquetol(imag(Stx(n,:)),1e-6);
        if numel(C_u) > numel(Stx_u)       %如果实际没用到很多星座点，这里会自动调整星座大小
            Stx_u = Stx_u(abs(Stx_u) == min(abs(Stx_u)));
            C_u = C_u(abs(C_u) == min(abs(C_u)));
        end
        fun = @(h) (h*Stx_u-C_u)*(h*Stx_u-C_u)';%从向量的角度讲，就是找到传输星座图和标准星座图差距向量模值最小的传输星座图系数。
        scaleFactor = fminsearch(fun,1);%从1开始求fun函数最小值点
        Stx(n,:) = Stx(n,:) * scaleFactor;          %相当于没做操作
    end
end

%% Transmitted Symbols
txSyms = signal2symbol(Stx,C,[],useGPU);%从星座点复数值到星座点索引值

%% Calculate Centroids of Rx Signal
if use_centroids
    for n = 1:nPol
        [centroids,Stx(n,:)] = getCentroids(Srx(n,:),Stx(n,:),C(:,n));   %没用到
        C(:,n) = centroids.';
    end
end
...
switch normMethod
    ...
    case 'MMSE'
        for n = 1:nPol
            fun = @(h) (h*Stx(n,:)-Srx(n,:))*(h*Stx(n,:)-Srx(n,:))';%发送和接收之间的差距向量
            c(n) = fminsearch(fun,1);%得到131072整体最小均方误差的那个h值，想象为stx诸多个星座点向量在空间上膨胀或者收缩，当MMSE最小时停下来定住，这一步是很必要的，因为脉冲成型改变了幅值，如果不做尺度上的伸缩，EVM会很大
        end
    case 'minSER'
...
for n = 1:nPol
    Stx(n,:) = Stx(n,:) * c(n);
    C(:,n) = C(:,n) * c(n);%发送和星座以和接收向量均方差最小为原则，所有星座以一定比例同等放缩
end

%% Demap Rx Symbols
if getSymsRX
    rxSyms = signal2symbol(Srx,C,[],useGPU);%判决完毕
end

%% Calculate Noise Variance
if exist('DEMAPPER','var') && isfield(DEMAPPER,'N0')&&(DEMAPPER.N0~=0)%最后这个条件是保证清零后仍然正常运行
    N0 = DEMAPPER.N0;
else
    N0 = getN0_MMSE(Stx,Srx);             %基于MMSE准则求噪声方差
end
Prx = var(Srx.');
N0 = N0 .* Prx;
...
if calc_LLR
    for n = 1:nPol
        ...
        symProb = histcounts(txSyms(n,:),-0.5:1:M-0.5,'Normalization','prob').';%横坐标1到64，纵坐标-0.5到63.5，区间间隔为1，对落到每个区间内索引值统计并用概率归一化
        % Calculate LLRs:
        LLRs(:,n) = LLR_eval(Srx(n,:),N0(n),C(:,n),symProb);%没看懂
    end
end
...
if strcmp(decoding,'diff-quad')
    ...
    if getSymsRX
        DEMAPPER.rxBits = syms2bits_diffQuad(rxSyms,nBpS);%这个就是CCDM的解码函数
        rxSyms = bit2sym(DEMAPPER.rxBits,nBpS);
    end
else
    if ~mod(nBpS,1)&&nBpS~=0%考虑到为零的时候，添加后半个条件和下面txbits=0的操作
