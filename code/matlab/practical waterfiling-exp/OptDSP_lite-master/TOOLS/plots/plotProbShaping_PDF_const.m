...
%% Calculate Constellation Entropy  %根据实际输入的symbol以及其概率算熵，symbol数量很重要，越多才越接近实际分配
tmp = log2(symProb);
tmp(isinf(tmp)) = 0;
H = -sum(symProb.*tmp);
symProb = symProb*100;
...
for n = 1:ni
    if ~any(P(:,n))%只要某一行有值(any)的反面是整行都没值(~any)
        delCol(n) = true;
    else
        delCol(n) = false;
    end
end
P = P(~delRow,~delCol);%如果整条横或者竖的概率为零的点也是被抛弃的，所以就算你设定用星座点128，当你需求的熵太小，或者你输入的符号数太少导致熵太小，周围的点概率都是0，它会缩减到36qam甚至更小
M = numel(P);  %如果你是用的128星座，熵也是比较正常的大的，出来的图是144qam,因为128不是正方形星座，所以12乘12=144，但四个象限各4个点（共16）是概率为0 的
ni = sqrt(M);
nq = ni;
