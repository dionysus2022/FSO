...
class = [];
symbolOrder = 'gray';
% Assignment of input parameters: 输入有很多参数名及其值，把对应名的值取出来
if nargin == 1
    SIG = varargin{1};
    M = SIG.M;
    if isfield(SIG,'nPol')
...
if strcmp(modulation,'QAM') && isempty(class) %如果是QAM又没有指定是方型还是星型
    if mod(sqrt(M),1) == 0
        class = 'square';
    else
        class = 'cross';
...
try
    ...
    switch modulation
        case 'QAM'
            ...
            const = qammod(symbolMap,M,symbolOrder); %symbolOrder指的是grey,qammod作用是把十进制星座索引变成复数星座点
            %scatterplot(const);%画星座点图
        case 'PAM'
...
symbolInd = zeros(1,M);
for n = 0:M-1
    symbolInd(n+1) = find(symbolMap==n);%symbolInd是星座索引，值是1至256
end
