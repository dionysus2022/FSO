"""Targeted 15 dB experiment with N=64, CP=8 — short CP scenario."""
...
def parse_args():
    ...
    parser.add_argument(
        "--output-dir",
        type=Path,
        default=root / "cp_fft_15db_target_N64_CP8_20260625",
    )
...
def main():
    ...
    print("=" * 60, flush=True)
    print(f"Experiment: N={config.n_subcarriers}, CP={config.cp_length}", flush=True)
    print(f"  CP / N = {config.cp_length / config.n_subcarriers:.4f}", flush=True)
    print(f"  symbol_length = {config.n_subcarriers + config.cp_length}", flush=True)
    ...
    for name, model in models.items():
        ...
        save_confusion(
            ...
            test["y"],
            test_prediction,
            f"{name}, blind CP-FFT+Merge, 15 dB, N=64 CP=8",
        )
        print(
    ...
    metrics = {
        ...
        "condition": f"N={config.n_subcarriers}, CP={config.cp_length} (N/8), CFO=0, STO=0, no turbulence",
        ...
        "results": results,
        "claim_scope": (
            "Short CP (CP = N/8) scenario; CP merging gain is significantly "
            "reduced compared to N=16, CP=16. Results reflect realistic short-CP "
            "degradation under otherwise ideal conditions."
        ),
    }
