# -*- coding: utf-8 -*-
"""
sim_compare_distributions_v2_1.py — S1-v2.1 分布检查

基于 sim_compare_distributions_v2.py，仅修改数据路径为 sim_data_v2_1。

S1-v2.1 判断标准（同 v2，来自 01_29.txt）:
...
输出: results/stage2/distribution_check_v2_1/
"""
...
# 路径与常量
# ─────────────────────────────────────────────────────────
SIM_MANIFEST_V21_CSV = STAGE2_ROOT / "sim_data_v2_1" / "manifest_sim_v2_1.csv"
DIST_CHECK_V21_ROOT = STAGE2_ROOT / "distribution_check_v2_1"
...
def plot_papr_by_mod(real_stats: dict, sim_stats: dict):
    ...
    fig.suptitle("Fig 1: PAPR_max & PAPR_p999 by Modulation (Real vs Sim-v2.1)", fontsize=14)
    ...
    for metric_key, metric_label, (ax_weak, ax_strong) in metrics:
        for j, (turb, ax) in enumerate(zip(REAL_TURBS, [ax_weak, ax_strong])):
            ...
            ax.bar(x + width/2, sim_vals, width, label="sim_v2.1", color="coral", alpha=0.8)
            ax.set_xticks(x)
    ...
    plt.tight_layout()
    path = DIST_CHECK_V21_ROOT / "fig1_papr_by_mod.png"
...
def plot_outlier_rate(real_stats: dict, sim_stats: dict):
    ...
    fig.suptitle("Fig 2: Outlier Rate by Modulation (Real vs Sim-v2.1)", fontsize=14)
    ...
    plt.tight_layout()
    path = DIST_CHECK_V21_ROOT / "fig2_outlier_rate.png"
    fig.savefig(path, dpi=120, bbox_inches="tight")
    plt.close(fig)
    print(f"  [OK] {path.name}")
...
def plot_amp_distribution(all_real: dict, all_sim: dict):
    ...
    fig.suptitle("Fig 3: amp_mean & amp_std Distribution (Real=blue, Sim-v2.1=red)", fontsize=14)
    ...
    for j, turb in enumerate(REAL_TURBS):
        ...
        ax_mean.hist(sim_amps, bins=30, alpha=0.5, color="red", label="sim_v2.1", density=True)
        ...
        ax_std.hist(sim_stds, bins=30, alpha=0.5, color="red", label="sim_v2.1", density=True)
    ...
    plt.tight_layout()
    path = DIST_CHECK_V21_ROOT / "fig3_amp_distribution.png"
    fig.savefig(path, dpi=120, bbox_inches="tight")
    plt.close(fig)
    print(f"  [OK] {path.name}")
...
def plot_radial_histogram(all_real: dict, all_sim: dict):
    ...
    fig.suptitle("Fig 4: Radial Histogram (Real=solid, Sim-v2.1=dashed)", fontsize=14)
    ...
    plt.tight_layout()
    path = DIST_CHECK_V21_ROOT / "fig4_radial_histogram.png"
    fig.savefig(path, dpi=120, bbox_inches="tight")
    plt.close(fig)
    print(f"  [OK] {path.name}")
...
def plot_constellation_overlay(all_real: dict, all_sim: dict):
    ...
    fig.suptitle("Fig 5: Constellation Overlay (Real=blue, Sim-v2.1=red)", fontsize=16, y=1.01)
    ...
    for i, mod in enumerate(MOD_NAMES):
        for j, turb in enumerate(REAL_TURBS):
            ...
            if sim_samples:
                ...
                ax.scatter(z_sim.real, z_sim.imag, s=1, c="red", alpha=0.3, label="sim_v2.1")
    ...
    plt.tight_layout()
    path = DIST_CHECK_V21_ROOT / "fig5_constellation_overlay.png"
    fig.savefig(path, dpi=120, bbox_inches="tight")
    plt.close(fig)
    print(f"  [OK] {path.name}")
...
def plot_cdm_comparison(all_real: dict, all_sim: dict):
    ...
    fig.suptitle("Fig 6: CDM Comparison (left=Real avg, right=Sim-v2.1 avg)", fontsize=14)
    ...
    for i, mod in enumerate(MOD_NAMES):
        for j, turb in enumerate(REAL_TURBS):
            ...
            if sim_samples:
                ...
                ax_sim.set_title(f"Sim-v2.1 {mod}/{turb}", fontsize=8)

    plt.tight_layout()
    path = DIST_CHECK_V21_ROOT / "fig6_cdm_comparison.png"
    fig.savefig(path, dpi=120, bbox_inches="tight")
    plt.close(fig)
    print(f"  [OK] {path.name}")
...
# ─────────────────────────────────────────────────────────
def main():
    print(f"=== sim_compare_distributions_v2_1.py  [{now_str()}] ===")
    ensure_dir(DIST_CHECK_V21_ROOT)
    ...
    for turb in REAL_TURBS:
        ...
        for mod in MOD_NAMES:
            ...
            sim_s = load_samples(SIM_MANIFEST_V21_CSV, sim_turb, mod, N_SAMPLES)
    ...
    ks_path = DIST_CHECK_V21_ROOT / "ks_metrics.json"
    ...
    # 写 summary
    lines = [
        f"Stage II S1-v2.1 分布检查 — {now_str()}",
        f"",
        f"=== S1-v2.1 判断标准（01_29.txt）===",
        ...
        f"",
        f"=== PAPR_max / PAPR_p999 / amp_mean / outlier by mod ===",
    ]
    for turb in REAL_TURBS:
        for mod in MOD_NAMES:
    ...
    summary_path = DIST_CHECK_V21_ROOT / "summary.txt"
    with open(summary_path, "w", encoding="utf-8") as f:
        f.write("\n".join(lines))
    print(f"\n  [OK] {summary_path.name}")

    print(f"\n[完成] S1-v2.1 分布检查已输出到 {DIST_CHECK_V21_ROOT}")
