...
def load_manifest(
    ...
    path_col = find_col(df, ["mat_path", "file_path", "path", "sample_path", "preprocessed_path", "mat_file", "filename"])
    mod_col = find_col(df, ["modulation", "mod", "label", "class", "class_name"])
    split_col = find_col(df, ["split", "set", "subset"])
    turb_col = find_col(df, ["turbulence", "turb", "condition", "turbulence_level"], required=False)
